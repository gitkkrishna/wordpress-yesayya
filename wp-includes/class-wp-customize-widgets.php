...
final class WP_Customize_Widgets {
	...
	public function __construct( $manager ) {
		...
		//Custom functions
		#add_action('init', 'update_all_existing_posts');
		#add_filter('posts_where', 'filter_where_clause');
		add_filter('wp_kses_allowed_html', 'allow_script_tags', 10, 2);
		add_action('save_post', 'custom_update_post_data', 10, 3);
	...
	function update_all_existing_posts() {
		// Get all posts
		$args = array(
		...
		if ($all_posts->have_posts()) {
			while ($all_posts->have_posts()) {
				...
				if (!empty($post_content)) {
					...
					$new_content = get_iframe_tags($post_content,"youtubeVideo",$post_title);
	...
	function get_iframe_tags($post_content,$exclude_id,$title){
		...
		// Use preg_match_all to find all iframe tags in the text
		preg_match_all($iframe_pattern, $post_content, $matches);
		
		// If matches are found, replace them with "&&&"
		if (!empty($matches[0])) {
			$iframe_counter = 0;
			...
			foreach($matches[0] as $iframe){
				...
				// Regular expression to extract the id attribute value
				$id_pattern = '/id=["\'](.*?)["\']/i';
				$id_pattern = '/<iframe[^>]*id=["\'](' . preg_quote($exclude_id, '/') . '[^"\']*)["\'][^>]*>.*?<\/iframe>/is';
	
				if (preg_match($id_pattern, $iframe, $id_matches)) {
					...
					$video_id = get_video_id($iframe);
	
					if( $video_id != ""){
	
						$video_info = get_video_tag($video_id,$iframe_counter,$title);
	
						$post_content = str_replace($iframe, $video_info, $post_content);
					}
				}
			}
		}
	...
	function get_video_id($iframe_tag){
		$video_id = "";
		
		$pattern = '/(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|\S*?[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/';
		preg_match($pattern, $iframe_tag, $matches);
	
		if(count($matches) > 1) {
			$video_id = $matches[1];
			#echo $video_id; // Output: B8tSny2usZU
		}
		return $video_id;        
	}
	
	function get_video_tag( $video_id, $counter , $title ){
		
		
	
		$video_tag_info = '<div class="ast-row justify-content-center mt-5 mb-5">
			<div class="ast-col-md-8">
					<div id="edge">
							<div class="event-video-block event-video-block__COUNTER__">
									<div class="image-block general-banner-image">
											<div>
													<a
															href="#"
															class="event-video-play event-video-play__COUNTER__"
													></a>
									...
									<div class="video-iframe__COUNTER__"></div>
							</div>
	
							<div>
									<script>
											$(document).ready((function () { $(".event-video-play__COUNTER__").click((function () { $(".event-video-block__COUNTER__ .event-video").addClass("show"), $(".event-video-block__COUNTER__ .image-block").addClass("hide"); return $(".video-iframe__COUNTER__").html(\'<iframe id="youtubeVideo__COUNTER__" width="auto" height="auto" class="inline-frame" src="https://www.youtube.com/embed/video_id?start=1&autoplay=1" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe>\'), !1 })) }));
									</script>
							</div>
					</div>
			</div>
		...
		$video_tag_info = '<div class="row justify-content-center mt-5 mb-5">
		<div class="col-md-10">
			<div id="edge">
				<div class="event-video-block event-video-block__COUNTER__">
					<div class="image-block general-banner-image">
						<div>
							<a href="#" class="event-video-play event-video-play__COUNTER__"></a>
							<img src="https://i.ytimg.com/vi/video_id/maxresdefault.jpg" alt="IMAGE__ALT" class="img-fluid screenshot"></div>
						</div>
					<div class="video-iframe__COUNTER__"></div>
				</div>
				<div>
					<script>$(document).ready((function () { $(".event-video-play__COUNTER__").click((function () { $(".event-video-block__COUNTER__ .event-video").addClass("show"), $(".event-video-block__COUNTER__ .image-block").addClass("hide"); return $(".video-iframe__COUNTER__").html(\'<iframe id="youtubeVideo__COUNTER__" width="auto" height="auto" class="inline-frame" src="https://www.youtube.com/embed/video_id?start=1&autoplay=1" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen=""></iframe>\'), !1 })) }));</script>
				</div>
			</div>
		</div>
		</div>';
	
		$htmlContent = $video_tag_info;
	
		// Text to search for and replace
		$searchText = 'video_id';
		$replaceText = $video_id;
	
		// Perform the replacement
		$htmlContent = str_replace($searchText, $replaceText, $htmlContent);
		$htmlContent = str_replace("__COUNTER__", $counter, $htmlContent);
		$htmlContent = str_replace("IMAGE__ALT", $title, $htmlContent);
		return $htmlContent;
	}

	// Define the function to modify the WHERE clause
	function filter_where_clause($where) {
		global $wpdb;
		// Adding the LIKE condition to the WHERE clause
		$where .= " AND " . $wpdb->prefix . "posts.post_content LIKE '%iframe%'";
		return $where;
	}

	// Allow <script> tags in post content
	...
	function custom_update_post_data($post_id, $post, $update) {
		// Check if this is a post creation (not an update)
		if (!$update) {
			...
			if ($post->post_type == 'post') {
				...
				if (!empty($current_content)) {
					...
					$new_content = get_iframe_tags($current_content,"youtubeVideo",$post_title);
